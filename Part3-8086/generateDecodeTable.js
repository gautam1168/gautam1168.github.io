...
fs.open("decodetable1byte.txt", "w", (err, fd) => {
	if (err) {
		console.error("Failed to open file", err);
		return;
	}
	
	for (let OpcodeIndex = 0; OpcodeIndex <= 255; ++OpcodeIndex) {
		
		Instruction = OpcodeIndex.toString(2).padStart(8, '0');
		Instruction += " " + getAssemblyTemplate1Byte(OpcodeIndex);
		Instruction += "\n";
...
fs.open("decodetable2byte.txt", "w", (err, fd) => {
	if (err) {
		console.error("Failed to open file", err);
		return;
	}
	...
	for (let OpcodeIndex = 0; OpcodeIndex <= 65536; ++OpcodeIndex) {
		...
		Instruction = OpcodeIndex.toString(2).padStart(16, '0');
		Instruction += " " + getAssemblyTemplate2Byte(OpcodeIndex);
		Instruction += "\n";
...
function immediateToRegister(OpName, FirstByte) {
	const w = (FirstByte & 0b00001000) >> 3;
	const reg = FirstByte & 0b111;
	return OpName + " " + regName[w][reg] + ", " + (w == 0 ? "{bytes} ;1" : "{bytes} ;2");
}

function accumulatorToMemory(OpName, FirstByte) {
	const w = (FirstByte & 0b1);
	return `${OpName} [{bytes}], ${ w == 0 ? 'AL' : 'AX' } ;2`;
}

function memoryToAccumulator(OpName, FirstByte) {
	const w = (FirstByte & 0b1);
	return `${OpName} ${ w == 0 ? 'AL' : 'AX' }, [{bytes}] ;2`;
}

function immediateToAccumulatorAdd(OpName, FirstByte) {
	const w = (FirstByte & 0b1);
...
function opToRegister(OpName, FirstByte) {
	const reg = (FirstByte & 0b111);
...
function getAssemblyTemplate1Byte(OpcodeIndex) {
	...
	if (FirstByte == 0b01110101) {
		return `JNE {bytes} ;1`;
	} else if (FirstByte == 0b01110100) {
		return `JE {bytes} ;1`;
	} else if (FirstByte == 0b01111100) {
		return `JL {bytes} ;1`;
	} else if (FirstByte == 0b01111110) {
		return `JLE {bytes} ;1`;
	} else if (FirstByte == 0b01110010) {
		return `JB {bytes} ;1`;
	} else if (FirstByte == 0b01110110) {
		return `JBE {bytes} ;1`;
	} else if (FirstByte == 0b01111010) {
		return `JP {bytes} ;1`;
	} else if (FirstByte == 0b01110000) {
		return `JO {bytes} ;1`;
	} else if (FirstByte == 0b01111000) {
		return `JS {bytes} ;1`;
	} else if (FirstByte == 0b01111101) {
		return `JNL {bytes} ;1`;
	} else if (FirstByte == 0b01111111) {
		return `JG {bytes} ;1`;
	} else if (FirstByte == 0b01110011) {
		return `JNB {bytes} ;1`;
	} else if (FirstByte == 0b01110111) {
		return `JA {bytes} ;1`;
	} else if (FirstByte == 0b01111011) {
		return `JNP {bytes} ;1`;
	} else if (FirstByte == 0b01110001) {
		return `JNO {bytes} ;1`;
	} else if (FirstByte == 0b01111001) {
		return `JNS {bytes} ;1`;
	} else if (FirstByte == 0b11100010) {
		return `LOOP {bytes} ;1`;
	} else if (FirstByte == 0b11100001) {
		return `LOOPZ {bytes} ;1`;
	} else if (FirstByte == 0b11100000) {
		return `LOOPNZ {bytes} ;1`;
	} else if (FirstByte == 0b11100011) {
		return `JCXZ {bytes} ;1`;
	} else if (FirstByte == 0b11010111) {
		return `XLAT ;0`;
	} else if (FirstByte == 0b10011111) {
		return `LAHF ;0`;
	} else if (FirstByte == 0b10011110) {
		return `LAHF ;0`;
	} else if (FirstByte == 0b10011100) {
		return `PUSHF ;0`;
	} else if (FirstByte == 0b10011101) {
		return `POPF ;0`;
	} else if (FirstByte == 0b00110111) {
		return `AAA ;0`;
	} else if (FirstByte == 0b00100111) {
		return `DAA ;0`;
	} else if (FirstByte == 0b00111111) {
		return `AAS ;0`;
	} else if (FirstByte == 0b00101111) {
		return `DAS ;0`;
	} else if (FirstByte == 0b10011000) {
		return `CBW ;0`;
	} else if (FirstByte == 0b10011001) {
		return `CWD ;0`;
	} else if (FirstByte == 0b11000011) {
		return `RET ;0`;
	} else if (FirstByte == 0b11001101) {
		return `INT {bytes} ;1`;
	} else if (FirstByte == 0b11001110) {
		return `INTO ;0`;
	} else if (FirstByte == 0b11001111) { 
		return `IRET ;0`;
	} else if (FirstByte == 0b11111000) {
		return `CLC ;0`;
	} else if (FirstByte == 0b11110101) {
		return `CMC ;0`;
	} else if (FirstByte == 0b11111001) {
		return `STC ;0`;
	} else if (FirstByte == 0b11111100) {
		return `CLD ;0`;
	} else if (FirstByte == 0b11111101) {
		return `STD ;0`;
	} else if (FirstByte == 0b11111010) {
		return `CLI ;0`;
	} else if (FirstByte == 0b11111011) {
		return `STI ;0`;
	} else if (FirstByte == 0b11110100) {
		return `HLT ;0`;
	...
	else if (FirstSevenBits == 0b1010000) {
		return memoryToAccumulator("MOV", FirstByte);
	} 
	// Immediate to register
	// 1011,w,reg 	data		data-1 if w=1
	else if (FirstNibble == 0b1011) {
		return immediateToRegister("MOV", FirstByte);
		/*
		const w = 0;
		const reg = 0b000;
	...
	else if (FirstSevenBits == 0b1010001) {
		return accumulatorToMemory("MOV", FirstByte)
	} 
	else if (FirstByte == 0b11000010) {
		return `RET {bytes} ;2`;
	}
	// Xchange register with accumulator
	else if ((FirstByte >> 3) == 0b10010) {
		const reg = (FirstByte & 0b111);
		return `XCHG AX, ${regName[1][reg]} ;0`;
	}
	else if ((FirstByte >> 3) == 0b01000) {
		const reg = (FirstByte & 0b111);
		return `INC ${regName[1][reg]} ;0`;
	}
	else if ((FirstByte >> 3) == 0b01001) {
		const reg = (FirstByte & 0b111);
		return `DEC ${regName[1][reg]} ;0`;
	}
	// Add immediate to accumulator
	// 0000010,w 	data 	dataifw=1 
	else if (FirstSevenBits == 0b10) {
		return immediateToAccumulatorAdd("ADD", FirstByte);
	} else if (FirstSevenBits == 0b0010110) {
		return immediateToAccumulatorAdd("SUB", FirstByte);
	} else if (FirstSevenBits == 0b0011110) {
		return immediateToAccumulatorAdd("CMP", FirstByte);
	} else if (FirstSevenBits == 0b0001010) {
		return immediateToAccumulatorAdd("ADC", FirstByte);
	} else if (FirstSevenBits == 0b0001110) {
		return immediateToAccumulatorAdd("SBB", FirstByte);
	} else if (FirstSevenBits == 0b0010010) {
		return immediateToAccumulatorAdd("AND", FirstByte);
	} else if (FirstSevenBits == 0b1010100) {
		return immediateToAccumulatorAdd("TEST", FirstByte);
	} else if (FirstSevenBits == 0b0000110) {
		return immediateToAccumulatorAdd("OR", FirstByte);
	} else if (FirstSevenBits == 0b0011010) {
		return immediateToAccumulatorAdd("XOR", FirstByte);
	} else if (FirstSevenBits == 0b1110010) {
		const w = FirstByte & 0b1;
		return `IN ${w == 0 ? 'AL' : 'AX'}, {bytes} ;-1`;
	} else if (FirstSevenBits == 0b1110011) {
		const w = FirstByte & 0b1;
		return `OUT {bytes}, ${w == 0 ? 'AL' : 'AX'} ;-1`;
	} else if (FirstSevenBits == 0b1110110) {
		const w = FirstByte & 0b1;
		return `IN ${w == 0 ? 'AL' : 'AX'}, DX ;0`;
	} else if (FirstSevenBits == 0b1110111) {
		const w = FirstByte & 0b1;
		return `OUT DX, ${w == 0 ? 'AL' : 'AX'} ;0`;
	}
	else if ((FirstByte >> 3) == 0b01010) {
		return opToRegister("PUSH", FirstByte);
	}
	else if ((FirstByte >> 3) == 0b01011) {
		return opToRegister("POP", FirstByte);
	}
	else if (((FirstByte & 0b11100000) == 0) && 
		((FirstByte & 0b111) == 0b110)) {
		return opToSegment("PUSH", FirstByte);
	}
	else if (((FirstByte & 0b11100000) == 0) && 
		((FirstByte & 0b111) == 0b111)) {
		return opToSegment("POP", FirstByte);
	}
	return "";
}

function getAssemblyTemplate2Byte(OpcodeIndex) {
	...
	if (FirstByte == 0b11010100 && SecondByte == 0b00001010) {
		return `AAM ;0`;
	...
	*/
	...
	// 100000,s,w  	mod,reg,rm 	disp-lo   disp-hi 	data 	dataifw=1
		...
		} else if (FirstSevenBits == 0b1000000 && reg == 0b100) {
